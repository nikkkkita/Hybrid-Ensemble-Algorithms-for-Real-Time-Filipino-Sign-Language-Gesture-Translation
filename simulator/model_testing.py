...
# KNN Function to find the 8 nearest classes for a given data point
def find_nearest_classes(data_point, X, y):
    distances = []
...
def main():
    ...
    # Load the dataset
    X, y = load_dataset('model/keypoint_classifier/keypoint.csv')
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id != -1:
                  ...
                  nearest_classes = find_nearest_classes(data_point, X, y)
                  ...
                  # Convert class labels back to numeric format if needed
                      ...
                      nearest_data_points.append(X[y == cls_label][0]) 

                # Use the trained Random Forest classifier to predict the class
                  predicted_class = rf_classifier.predict(nearest_data_points)
                  predicted_class_index = predicted_class[0]  # Index of the predicted class
                  probabilities = rf_classifier.predict_proba(nearest_data_points)
                  predicted_class_probability = probabilities[0][predicted_class_index]  # Probability of the predicted class
                  print("Probabilities of the 8 nearest classes:")
                  for i, cls in enumerate(nearest_classes):
                   print(f"{cls}: {probabilities[0][i]}")
                  print("Random Forest predicted class:", keypoint_classifier_labels[predicted_class_index])
